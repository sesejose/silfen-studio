...
function showBag(bag) {
  ...
  document.querySelector(".test").src = bag.thumb1.guid;

  // document.querySelector(".product-img-shop a").style.backgroundImage = `url(${bag.thumb1.guid})`;

  copy
    .querySelector(".price-add-to-card a")
    .setAttribute("href", `product.html?id=${bag.id}`);

  // copy.querySelector(".product-image-shop a").setAttribute("href", `product.html?id=${bag.id}`);
